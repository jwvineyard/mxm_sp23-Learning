...
class ModelBasedQLearningAgent:
    ...
    def select_action(self, state):
        if np.random.rand() < self.epsilon:
            # Explore: choose an action based on transition probabilities
    ...
    def explore_action(self, state):
        # Get the transition probabilities for each action
        transitions_left = self.env._get_transition_probabilities(state, 0)
        transitions_right = self.env._get_transition_probabilities(state, 1)

        # Calculate the expected value for each action
        q_left = sum(prob * self.q_values[next_state, np.argmax(self.q_values[next_state])] for next_state, prob in transitions_left)
        q_right = sum(prob * self.q_values[next_state, np.argmax(self.q_values[next_state])] for next_state, prob in transitions_right)
